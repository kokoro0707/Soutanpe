...
public class HPBarSlider : MonoBehaviour
{
    ...
    private void Update()
    {
        hpSlider.SetValueWithoutNotify(target.currentHP);
        ...
        if (damageSlider.value > hpSlider.value)
        {
            ...
            if (timer >= delay)
            {
                damageSlider.SetValueWithoutNotify(
                    Mathf.MoveTowards(damageSlider.value, hpSlider.value, speed * Time.deltaTime));
            }
        }
        else
        {
